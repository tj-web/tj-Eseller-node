...
export async function getAllOrders({
  ...
  const dataSql = `
    ...
    INNER JOIN (
        SELECT tord.order_id,
            ...
            po.licence
        FROM tbl_order tord
        JOIN tbl_order_product top 
            ON (top.order_id = tord.order_id AND top.vendor_id = :vendor_id)
        LEFT JOIN (
    ...
    JOIN tbl_product_image tpi 
        ON (tpi.product_id = top.product_id AND tpi.default = 1 AND tpi.status = 1)
    LEFT JOIN order_product_sales_detail ops 
        ON (ops.order_id = top.order_id AND ops.product_id = top.product_id)
  ...
  const countSql = `
    ...
    WHERE top.vendor_id = :vendor_id
    ${order_status == 1
      ? "AND tord.order_status NOT IN (5,11,17)"

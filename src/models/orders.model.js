...
export async function getAllOrders({
  ...
  if (order_status) {
    if (order_status == 5) {
      whereCondition = "AND tord.order_status IN (5, 11, 17)";
    } else if (order_status == 1) {
      whereCondition = "AND tord.order_status NOT IN (5, 11, 17)";
    }
  } else {
    whereCondition = "AND tord.order_status IN (5, 11, 17)";
  }
  ...
  const sql = `
    ...
    INNER JOIN tbl_order tord 
        ON tord.order_id = top.order_id
    ...
    LEFT JOIN (
        ...
        ON po.order_id = tord.order_id

    LEFT JOIN tbl_product_image tpi 
        ON tpi.product_id = top.product_id 
        AND tpi.default = 1 
        AND tpi.status = 1

    LEFT JOIN order_product_sales_detail ops 
        ON ops.order_id = top.order_id 
        AND ops.product_id = top.product_id
